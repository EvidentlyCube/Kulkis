...
public class TPlayer extends TGameObject {
    ...
    override public function update():void {
        ...
        const SpdXModifier:Number = (Level.isHardMode ? 0.1 : 0);
        // Horizontal movement
        if (RetrocamelInputManager.isKeyDown(Game.keyLeft)) {
            x -= SPD_X+SpdXModifier;
    ...
    public function kill():void {
        ...
        if (Level.isHardMode){
            RetrocamelEffectQuake.make().mode(RetrocamelEffectQuake.MODE_RANDOM).power(20, 20).duration(250).run();
            onKillQuakeFinished();
        } else {
    ...
    private function onKillQuakeFinished():void {
        if (Level.isHardMode){
            RetrocamelEffectFadeScreen.makeOut().duration(250).callback(onKillFadeFinished).run();
        } else {
